...
void Control(void){
    ...
    if(!debug_stop){
        if(right_button.button);
			fps++;
//				int8_t speed=20;//0~30
//        EncoderCallBack();
//        Loutput1 = PID_Compute(&hpid_lmotor, speed, encoderL_10ms);
//        Routput1 = PID_Compute(&hpid_rmotor, speed, encoderR_10ms);
//        SetMotorOutput(Loutput1, Routput1);

				IntercomCallBack();
        FollowLinePIDCallBack(27);
//				TaskManagerCallBack();
//				Turn(TU_CW,1);
        MedicineBoxCallBack();
			
        
        if(loaded == 0) Ignite(Y);
        else Ignite(G);
    }
    else{
        GrayScalerCallBack();
